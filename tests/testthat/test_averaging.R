context("Average technical replicates")
...
my_files <- list.files(path = "tests/testthat/average",
                       pattern = ".csv",
                       full.names = TRUE)
my_filenames <- list.files(path = "tests/testthat/average",
                           pattern = ".csv",
                           full.names = FALSE)
...
my_results <- lapply(my_loaded_datasets, average_technical_replicates, ttn, blk)

# Test whether results are identical to pre-computed expected results
test_that("average_technical_replicates gives correct results", {
