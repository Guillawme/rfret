...
`format_data()` function to prepare the raw data.
...
It also needs a parameter `data_type = "fret"` to indicate that we want to
pre-process FRET data (because `rfret` can also handle FP data, and can be
...
The function `inspect_raw_data()` takes the output of `format_data()`
...
`format_data()` in case there are no replicates, and outputs the corrected 
