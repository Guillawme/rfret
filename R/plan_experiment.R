...
#'
#' @param kd A Kd value. This can also be a numerical vector of values, and in
#'     this case the function will plot several binding curves on the same
#'     graph: one for each Kd value.
#' @param min_concentration The minimal value of the concentration range to
#'     simulate the binding curve over. Defaults to \code{1e-1}.
#' @param max_concentration The maximal value of the concentration range to
#'     simulate the binding curve over. Defaults to \code{1000}.
#' @param binding_model A binding model equation. Possible values are
#'     \code{hyperbola}, \code{quadratic} and \code{quadratic_homodimer}.
#'     Defaults to \code{hyperbola}.
...
plan_experiment <- function(kd,
    ...
    if (is.null(probe_conc)) {
        if (!identical(binding_model, hyperbola)) {
    ...
    # Make plot
    my_plot <- ggplot2::ggplot(data = data.frame(min_concentration:max_concentration,
                                                 max_concentration:min_concentration),
                               ggplot2::aes(x = seq(from = min_concentration,
                                                    to = max_concentration))) +
