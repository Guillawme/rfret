...
#'     \code{signal_min} and \code{signal_max} (the parameters of the binding
#'     model equations), using the corrected FRET data.
#'
...
#'     \code{concentration}.
#' @return A list containing initial guesses for \code{kd}, \code{signal_min}
#'     and \code{signal_max}.
#' @export
...
guess_parameters <- function(fret_corr) {
    ...
    # for signa_min
    signal_min_guess <- min(fret_corr$fret_corrected)
    ...
    signal_max_guess <- max(fret_corr$fret_corrected)
    ...
    list(kd       = kd_guess,
         signal_min = signal_min_guess,
         signal_max = signal_max_guess)
}
